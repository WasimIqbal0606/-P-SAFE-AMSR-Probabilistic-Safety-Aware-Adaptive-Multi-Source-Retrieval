...
Table~\ref{tab:statistics} reports Holm-Bonferroni corrected significance tests and formal non-inferiority testing against Deep Hybrid. Primary improvements over Dense are significant after Holm correction on SciFact High Recall ($p_{\mathrm{Holm}} = 0.0232$, $d_z = 0.23$, 95\% CI $[+0.0179, +0.0943]$), NFCorpus High Recall ($p_{\mathrm{Holm}} = 0.0378$, $d_z = 0.20$), and ArguAna Balanced ($p_{\mathrm{Holm}} = 0.0316$, $d_z = 0.10$).

Non-inferiority against Deep Hybrid ($\epsilon=0.010$) is statistically established for ArguAna Balanced (95\% LB = $-0.0043$, $p_{\mathrm{NI, Holm}} < 0.001$), ArguAna High Recall (95\% LB = $-0.0040$, $p_{\mathrm{NI, Holm}} < 0.001$), and NFCorpus High Recall (95\% LB = $-0.0066$, $p_{\mathrm{NI, Holm}} = 0.0212$). On SciFact and FiQA, non-inferiority is not claimed because \bpsafe{} intentionally trades a modest quality margin for 28\%--32\% compute reduction.
...
Table~\ref{tab:calibration} and Fig.~\ref{fig:calibration} confirm calibration quality. Brier scores for $P_{\mathrm{gain}}$ remain between $0.141$ and $0.218$, with ECE below $0.095$ and AUROC exceeding $0.78$ across datasets. For rare harm events ($P_{\mathrm{harm}}$), Brier scores remain below $0.135$ and calibration slopes remain close to the ideal $1.00$, supporting probabilistic risk gating.
...
\begin{figure}[t]
    ...
    \caption{B-P-SAFE Balanced vs 100-seed Matched-Budget Random baseline distribution with 95\% empirical confidence intervals on seed 42.}
    \label{fig:matched_budget}
\end{figure}

Figure~\ref{fig:matched_budget} compares \bpsafe{} against the matched-budget random baseline over 100 random allocations. On SciFact Balanced, \bpsafe{} reaches $0.6965$ versus $0.6676 \pm 0.0059$ (95\% CI $[0.6558, 0.6789]$); on ArguAna, \bpsafe{} achieves $0.4069$ versus $0.3939 \pm 0.0024$ (95\% CI $[0.3891, 0.3986]$). This confirms that \bpsafe{} successfully selects *which* queries benefit from escalation rather than gaining from arbitrary compute allocation.

\subsection{Stability: Training Stochasticity vs Split Sensitivity}

\input{tables/stability_table.tex}

Table~\ref{tab:stability} evaluates 10 independent training seeds on the fixed primary split (seed 42). Model fitting stochasticity is negligible ($SD = 0.0000$), demonstrating that router fitting is deterministic and stable under fixed data partitions. In contrast, split sensitivity across seeds 42, 123, and 2026 exhibits moderate standard deviations ($SD = 0.0020 - 0.0342$), confirming that query partition variance dominates algorithmic variance.
