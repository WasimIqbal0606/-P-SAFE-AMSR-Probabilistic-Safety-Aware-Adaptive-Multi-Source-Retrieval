...
reduction without statistically significant degradation. Performance
claims are evaluated using paired $t$-tests, Wilcoxon signed-rank
tests, bootstrap confidence intervals, and permutation tests.
...
\subsection{Per-Stage Inference Latency Breakdown}
\label{sec:latency}
...
Excluding BM25 lexical retrieval, the routing-specific overhead from
feature extraction and router decision is approximately $2.8\text{ ms}$.
Including the always-on BM25 stage, the non-reranking overhead remains
small relative to the $736.69\text{ ms}$ Cross-Encoder cost. The Cross-Encoder at $736.69\text{ ms}$ is
...
\begin{figure}[htbp]
    ...
    sub-4\,ms routing-specific overhead of feature extraction and router
...
\begin{equation}
\label{eq:valscore}
\mathcal{S}_{\text{val}} = 0.50\,\text{HGR}
  + 0.20\,\text{LS}
  + 0.15\,\text{RC}
  + 0.10\,\text{HA}
  + 0.05\,\text{OGC}
...
\begin{itemize}
  ...
  \item \textbf{Deep Hybrid} ($A_6^{\text{DeepHybrid}}$): Full
        ...
        documents. This represents the maximum-compute operating point and the
        expected upper-quality reference in domains where hybrid re-ranking is beneficial.
\end{itemize}
...
\textbf{HGR} (Hybrid Gain Retention vs.\ Deep Hybrid):
\begin{equation}
\text{HGR} = \frac{\bar{q}_{\text{P-SAFE}} - \bar{q}_{\text{dense}}}{\bar{q}_{\text{DeepHybrid}} - \bar{q}_{\text{dense}}}
\end{equation}
When Deep Hybrid does not improve over Dense, HGR is clipped to 0 and the dataset is treated as protection/no-benefit.
\textbf{LS} (Latency Saving, defined as the reduction in Cross-Encoder
re-ranking latency relative to always-on Deep Hybrid, not total wall-clock latency),
...
Hybrid quality (HGR). However, B-P-SAFE remains significantly below
...
worse than Deep Hybrid ($p = 0.442$), achieving $88.1\%$ hybrid gain
retention with $26.5\%$ re-ranking latency saving.
...
\begin{itemize}
    ...
    \item \textbf{Router baselines.} Current comparisons use the
          ...
          should benchmark against a margin-based dense confidence router,
          a BM25/dense disagreement router, a random router with matched
          activation rate, a regression-only router, a classifier-only
          router, and always-rerank / never-rerank bounds, along with
          cost-aware dispatchers such as FrugalGPT adapters.
