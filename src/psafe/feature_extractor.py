...
class FeatureExtractor:
    ...
    def _entropy(self, scores: np.ndarray, norm_k: int = None) -> float:
        """Softmax entropy over real-valued scores (handles negatives correctly)."""
        if len(scores) == 0:
            return 0.0
        scores = np.asarray(scores, dtype=np.float64)
        # shift by max for numerical stability — valid for any real-valued input
        scores = scores - np.max(scores)
        exp_s = np.exp(scores)
        p = exp_s / (np.sum(exp_s) + 1e-12)
