...
class BPSafeRouter:
    ...
    def tune_thresholds(self, val_data, candidate_counts_val=None, out_dir=None):
        """FIX 6: Validation-only threshold tuning across ALL non-dense actions."""
        candidate_counts_val = candidate_counts_val or {}
        best_u = -float('inf')
        ...
        for a in non_dense:
            ...
            action_lats_pred[a] = self.models_lat[a].predict(X_val)
        ...
        for g_th in gain_thresholds:
            for h_th in harm_thresholds:
                ...
                predicted_utility = np.zeros(n_val)
                ...
                for qi in range(n_val):
                    ...
                    for a in non_dense:
                        ...
                        pred_lat = float(action_lats_pred[a][qi])
                        cand_count = candidate_counts_val.get(a, 0)
                        
                        u = (pred_d
                             - self.lambda_latency * pred_lat
                             - self.lambda_harm * ph
                             - self.lambda_candidate * cand_count
                             + self.lambda_recovery * pg)
                    ...
                    predicted_utility[qi] = best_qi_u
                ...
                if mean_util > best_u:
                    ...
                    har = sum(a != Action.A0_DENSE.value for a in selected_actions) / n_val if n_val > 0 else 0
                    best_metrics = {
                        "hybrid_activation_rate": har,
                        "mean_predicted_utility": float(np.mean(predicted_utility)),
                        "mean_realized_delta": float(np.mean(realized_utility))
                    }
        ...
        if out_dir:
            ...
            with open(os.path.join(out_dir, "validation_tuning.json"), "w") as f:
                json.dump({
                    ...
                    "hybrid_activation_rate": best_metrics.get("hybrid_activation_rate", 0.0),
                    "mean_predicted_utility": best_metrics.get("mean_predicted_utility", 0.0),
                    "mean_realized_delta": best_metrics.get("mean_realized_delta", 0.0)
                }, f, indent=4)
    ...
    def route(self, features: np.ndarray, query_id: str,
        ...
        action_pred_lat = {}
        rejected_reasons = {}
        override_used_by_action = {}
        override_reason_by_action = {}
        ...
        for a in self.actions:
            ...
            if rejected and self.mode == "balanced":
                if delta_pred > 0.02 and p_harm < self.harm_threshold + 0.10:
                    ...
                    reject_reason = ""
                    override_used = True
                    override_reason = "balanced_soft_override"
            elif rejected and self.mode == "high_recall":
                if delta_pred > 0.005 and p_harm < self.harm_threshold + 0.20:
                    rejected = False
                    reject_reason = ""
                    override_used = True
                    override_reason = "high_recall_soft_override"
            # lite: no override
            
            override_used_by_action[a] = override_used
            override_reason_by_action[a] = override_reason
        ...
        self._action_predictions.append({
            ...
            "override_used": override_used_by_action.get(best_a, False),
            "override_reason": override_reason_by_action.get(best_a, ""),
            "mode": self.mode,
            "split": split,
        })
