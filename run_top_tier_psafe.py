...
from psafe.evaluation.evaluator import calculate_sensitivity_splits, analyze_sensitivity, EvaluationManager
...
def run_experiment(config: dict, dataset: str, profile: str, mode: str):
    print("=" * 80)
    ...
    mode_dir = os.path.join(out_dir, dataset, mode)
    ...
    evaluator = Evaluator(k_values=[10], relevance_threshold=rel_threshold) 
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/top_tier.yaml")
    parser.add_argument("--dataset", type=str, help="Override dataset")
    parser.add_argument("--multi-dataset", action="store_true", help="Run on all config datasets")
    parser.add_argument("--cross-dataset", action="store_true", help="Run in leave-one-out mode")
    parser.add_argument("--profile", type=str, help="Override model profile")
    parser.add_argument("--mode", type=str, help="Override router mode (lite, balanced, high_recall)")
    parser.add_argument("--modes", nargs="+", help="Multiple modes to run")
    
    args = parser.parse_args()
    config = load_config(args.config)
    
    datasets = [args.dataset] if args.dataset else config.get("datasets", [])
    if args.multi_dataset and not args.dataset:
        datasets = config.get("datasets", [])
        
    profiles = [args.profile] if args.profile else list(config.get("model_profiles", {}).keys())
    modes = args.modes if args.modes else ([args.mode] if args.mode else list(config.get("router_modes", {}).keys()))
    ...
    # Handle skipped baselines gracefully if FlagEmbedding not available
    from psafe.retrievers.bgem3_retriever import BGEM3MultiFunctionRetriever
    if not BGEM3MultiFunctionRetriever.is_available():
        BGEM3MultiFunctionRetriever.save_skipped_baselines(os.path.join(out_dir, "reports"))
    
    if args.cross_dataset:
        print("Running cross-dataset evaluation (leave-one-out)...")
    else:
        for ds in datasets:
            for prof in profiles:
                for md in modes:
                    try:
                        run_experiment(config, ds, prof, md)
                    except Exception as e:
                        print(f"Failed {ds}/{prof}/{md}: {e}")
